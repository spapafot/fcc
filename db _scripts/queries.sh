...
echo "$($PSQL "SELECT GREATEST(winner_goals,opponent_goals) as g FROM games ORDER BY g DESC LIMIT 1")"
...
echo "$($PSQL "SELECT name FROM games JOIN teams ON games.winner_id=teams.team_id OR games.opponent_id=teams.team_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name")"
